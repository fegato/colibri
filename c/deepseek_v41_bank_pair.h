/* deepseek_v41_bank_pair.h — decision logic for the double-buffered prefill
 ...
 * the CUDA glue in deepseek_v41.c wires threads and streams around it. */
#ifndef DEEPSEEK_V41_BANK_PAIR_H
#define DEEPSEEK_V41_BANK_PAIR_H

typedef enum {
    V41_BANK_SWAP = 0,        /* other bank holds this layer: swap roles */
    V41_BANK_LEGACY = 1,      /* refill the active bank on demand */
} V41BankPairAction;
...
static inline V41BankPairAction coli_v41_bank_pair_decide(int double_on,
                                                        int other_layer,
                                                        int incoming_layer) {
    if (!double_on) return V41_BANK_LEGACY;
    if (other_layer < 0 || other_layer != incoming_layer)
        return V41_BANK_LEGACY;
    return V41_BANK_SWAP;
}
 ...
 * path above absorbs. */
static inline int coli_v41_bank_pair_prefetch_target(int double_on,
                                                    int current_layer,
                                                    int num_layers) {
...
#endif /* DEEPSEEK_V41_BANK_PAIR_H */
