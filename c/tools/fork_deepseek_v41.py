...
def units_deltas(text: str) -> str:
    ...
    # V41 DELTA: the engram unit has no V4 counterpart -- it owns the n-gram hash
    # addressing, which V4 does not have (V4 routed by token-id hash instead).
    anchor = "V41_TARGET_UNITS = \\\n\tCOLI_V41_UNIT_ST \\\n"
    assert anchor in text, "unit list moved: update the fork script"
    if "COLI_V41_UNIT_ENGRAM" not in text:
        text = text.replace(anchor, anchor + "\tCOLI_V41_UNIT_ENGRAM \\\n")
    return text.replace("\n", "\r\n") if crlf else text
...
def main() -> int:
    ...
    parser.add_argument("--force", action="store_true",
                        help="also overwrite the hand-maintained sources (destroys "
                             "the V4.1 deltas in them)")
    ...
    for source_name, target_name in FILES:
        ...
        if args.check:
            ...
            continue
        # A hand-maintained file carries the V4.1 deltas on top of the fork, so
        # writing it here would throw that work away. Refuse unless the caller
        # says --force, and never pretend it was a no-op.
        if target_name in HAND_MAINTAINED and target.exists() and not args.force:
            print(f"refusing to overwrite hand-maintained {target_name} "
                  "(use --force to reset it to the bare fork)")
            continue
