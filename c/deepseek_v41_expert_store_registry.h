 ...
 *
 * The built-in "auto" backend (coli_v41_expert_store_open_planned) is registered
...
#include "deepseek_v41.h" /* ColiDeepSeekV41Config (engine-less CLI path passes it) */

#ifdef __cplusplus
extern "C" {
#endif

/* Forward declarations. The concrete structs live in deepseek_v41_internal.h;
...
typedef struct ColiDeepSeekV41ExpertStoreOptions ColiDeepSeekV41ExpertStoreOptions;
 ...
 * path, which has no ColiV41Engine) can rely on. `engine` may be NULL for
 * backends that only consume `config`; the "auto" backend requires a non-NULL
 * engine. Same return contract as coli_v41_expert_store_open_planned: zero on
